...
$string['helpertext'] = 'Help text for the category {$a}';
...
$string['questionneedattention'] = 'This question need your attention : elle n\'est pas complété correctement.';
...
$string['singlechoicemode'] = 'This magazine test is in singlechoice mode. You cannot write answer texts in this mode. the sudent will just answer "yes" or "no" to the question, and distribute weights in categories.';
...
$string['testnotallok'] = 'The configuration of this test is not completed : you cannot perform it.';
...
$string['youneedcreatingcategories'] = 'Before creating questions, you have to create at least two categories.';
...
Import file must be UTF8 encoded, separated with ";", one question per line and
have one first column for question text.

If singlechoice options is on, next columns are weights (integer) for each
category in category order.

If singlechoice is off, next columns must feed in order : answer text, weight
(1 if unweighted), and helpertext for each category in order.';

$string['modulename_help'] = 'The Magtest activity provides a multicriteria
evaluation summing a score on several scales for each answer. At the end of the
test, the student will be categorized in his dominating category. The teacher
can use results to create course groups.';

$string['usemakegroups_help'] = '
### Generating moodle groups from results

Using the multitrack test will result in a splitting of the participants among
the provided categories.

This setting allows activating the group generation feature that will convert
the test result into moodle groups within the course.

Generation can take place even if all participants did not answer the test. By
the way, there will not be any generation possible if groups already are set
within the course. You will have to delete the existing groups if you want to
generate groups again from a later attempt.';

$string['weighted_help'] = '## Multitrack test

### Weighted mode

If you enable this mode, each answer will be associated a weight that will be
added to the relevant category if choosen.

This allows creating tests in which each question may not have the same
influence upon the final result.

When weighted mode is on, the default weight is 1.0, which is equivalent not
to use weighting.';
...
This parameter allows choosing how many questions will be displayed per page.
Setting this value to 1 will display a single question page.';

$string['allowreplay_help'] = '## Multitrack test

### Allow test replay

this setting allows disabling or enabling globally the capability to make
multiple attempts of the test. Participants will need having the "Make multiple
attempts" capability enabled in their profile to get access to the replay button.';

$string['magtest_help'] = '
Magtest implements such magazine tests collecting answers "by category" of
answers. The test allows you to define some categories and deploy a set of
questions that will have as many proposals for each category.

When a participants plays the test, he will have to choose one of the possible
proposals, after what the category score will be increased.

At the end of the test, the winning category has collected the higher score. A
conclusion can be shown to users highlighting the winning sentence, and
followed eventually by an overal conclusion.

The resulting groups of participants, sorted by the winning category can
eventually be converted to Moodle groups. This module can thus be used as a
self-discrimating operator when starting a course session, or when splitting a
course population into groups becomes necessary.';

$string['singlechoice_help'] = 'If checked each question is requested only one
simple answer (selected or not). Weights apply to all marking categories for
the questions that are checked. Magtest is necessarily weighted in this case.';
$string['you_have_to_create_categories'] = 'You have to create at least one
category before you can create any questions.';
